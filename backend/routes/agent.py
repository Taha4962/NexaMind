"""
NexaMind Backend — Agent Chat & Streaming Routes
...
@router.post(
    "/chat/stream",
    summary="Send a message and receive an SSE stream of tokens",
    description="Streams responses token-by-token using Server-Sent Events (SSE).",
)
async def agent_chat_stream(
    ...
    # a. Save user message to MongoDB immediately before streaming
    chat_doc = await _get_or_create_chat(user_id, request.chatId)
    chat_id = str(chat_doc["_id"])
    is_new_chat = request.chatId is None or chat_doc.get("messageCount", 0) == 0
    ...
    # b. Fetch chat history (last 10 messages)
    history = await _fetch_recent_messages(chat_id, limit=10)

    # c. Run intent classification via Flash-Lite (fast, non-streaming)
    ...
    async def event_generator() -> AsyncGenerator[str, None]:
        ...
        if chunks:
            ...
            agent_type = AgentType.DIRECT
            groq_messages = _build_groq_messages(request.message, history)
...
async def _wrap_and_persist_stream(
    ...
    """Wraps SSE generator to save complete or partial message to DB on completion."""
    accumulated_content = []
...
async def _classify_intent(query: str, history: list[dict[str, Any]]) -> dict[str, Any]:
    """Fast non-streaming intent classification using Flash-Lite."""
    prompt = (
...
def _build_groq_messages(query: str, history: list[dict[str, Any]]) -> list[dict[str, Any]]:
...
async def _fetch_recent_messages(chat_id: str, limit: int = 10) -> list[dict[str, Any]]:
    cursor = (
        MongoDB.messages()
...
async def _call_direct(query: str, history: list[dict[str, Any]]) -> tuple[str, str]:
    messages = _build_groq_messages(query, history)
    try:
